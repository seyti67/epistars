<script>
	...
	onMount(() => {
		...
		var baseVertexShader = compileShader(
			gl.VERTEX_SHADER,
			'precision lowp float; precision lowp sampler2D; attribute vec2 aPosition; varying vec2 vUv; varying vec2 vL; varying vec2 vR; varying vec2 vT; varying vec2 vB; uniform vec2 texelSize; void main () {     vUv = aPosition * 0.5 + 0.5;     vL = vUv - vec2(texelSize.x, 0.0);     vR = vUv + vec2(texelSize.x, 0.0);     vT = vUv + vec2(0.0, texelSize.y);     vB = vUv - vec2(0.0, texelSize.y);     gl_Position = vec4(aPosition, 0.0, 1.0); }'
		);
		var clearShader = compileShader(
			gl.FRAGMENT_SHADER,
			'precision lowp float; precision lowp sampler2D; varying vec2 vUv; uniform sampler2D uTexture; uniform float value; void main () {     gl_FragColor = value * texture2D(uTexture, vUv); }'
		);
		var displayShader = compileShader(
			gl.FRAGMENT_SHADER,
			'precision lowp float; precision lowp sampler2D; varying vec2 vUv; uniform sampler2D uTexture; void main () {     gl_FragColor = texture2D(uTexture, vUv); }'
		);
		...
		var advectionShader = compileShader(
			gl.FRAGMENT_SHADER,
			'precision lowp float; precision lowp sampler2D; varying vec2 vUv; uniform sampler2D uVelocity; uniform sampler2D uSource; uniform vec2 texelSize; uniform float dt; uniform float dissipation; void main () {     vec2 coord = vUv - dt * texture2D(uVelocity, vUv).xy * texelSize;     gl_FragColor = dissipation * texture2D(uSource, coord); }'
		);
		...
		function update() {
			...
			for (var i = 0, len = pointers.length; i < len; i++) {
				...
				if (pointer.moved) {
					splat(pointer.x, pointer.y, pointer.dx, pointer.dy, colorArr);
					pointer.moved = false;
				}
			}
